...
func (s *InfluxDB) Query(query *monitor.MetricQuery) (*types.MetricMergedResult, error) {
	...
	for dbname, cmd := range metricsReq {
		...
		if res, err := s.queryAndCheck(dbname, cmd); err == nil {
	...
	if len(query.Conditions) > 0 {
		for _, v := range query.Conditions {
			if v.Key == "tke_cluster_instance_id" {
				if util.ParseClusterConditions(types.InfluxdbType, &v) {
					clusters = s.getAllClusterNames()
				} else {
					clusters = append(clusters, v.Value)
				}
				continue
			}
...
func (s *InfluxDB) queryAndCheck(dbname string, cmd string) (*influxclient.Response, error) {
...
func (s *InfluxDB) getAllClusterNames() []string {
	var clusterNames []string
	if res, err := s.queryAndCheck("", showDatabases); err == nil {
		clusterNames = parseShowDatabases(res)
	}
	return clusterNames
}

func parseShowDatabases(result *influxclient.Response) []string {
	var clusterNames []string
	if result.Results == nil || len(result.Results) == 0 {
		log.Errorf("Nil query result from %s", showDatabases)
		return clusterNames
	}
	for _, r := range result.Results {
		for _, s := range r.Series {
			for _, v := range s.Values {
				dbname := v[0].(string)
				if strings.HasPrefix(dbname, clusterNamePrefix) {
					clusterNames = append(clusterNames, dbname)
				}
			}
		}
	}
